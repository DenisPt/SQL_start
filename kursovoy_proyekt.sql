/* БД предназначена для расчета заказов поставщикам, включает в себя множество параметров, которые относятся отдельно к магазинам, отдельно к товарам,
отдельно к узлам товарного классификатора. По сути вся БД делается ради представления, которое создано в файле kursovoy_proyekt_view.sql */
